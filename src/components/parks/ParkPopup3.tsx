...
import styles from './ParkPopup3.module.scss';
...
const ParkPopup3: React.FC<ParkPopupProps> = ({
  activePark,
  onGetDirections,
  isLoadingDirections,
  directions,
    ...
    >
      ...
      <div className={styles.detailsContainer}>
        <div className={styles.details}>
          ...
          {/* Enhanced Info Pills - Compact Pill Style */}
          <div className={styles.infoPills}>
            {/* Park Rating Pill */}
            {parkRating && parkRating > 0 && (
              <div className={classnames(styles.pill, styles.ratingPill)}>
                <Stars rank={parkRating} size={12} />
                <span className={styles.pillText}>{parkRating.toFixed(1)}</span>
              </div>
            )}

            {/* Friends in Park Pill */}
            {userId && friendsInParkIds && friendsInParkIds.length > 0 && (
              <div className={classnames(styles.pill, styles.friendsPill)}>
                <Users size={14} />
                <span className={styles.pillText}>
                  {friendsInParkIds.length}
                </span>
                <span className={styles.pillLabel}>friend</span>
              </div>
            )}
